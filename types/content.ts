...
export type AdoptionLearningCompletenessDimension = HowDimension|"ACTIVATION CRITERIA"|"TTV DEFINITION"|"ADHERENCE CRITERIA"|"VALUE EVIDENCE"|"FEEDBACK SOURCE"|"LEARNING TRIGGER"|"DECISION LOOP"|"BUSINESS LINKAGE";
export type AdoptionSemanticTerm = "AVAILABILITY"|"ACTIVATION"|"USAGE"|"ADOPTION"|"ADHERENCE"|"CUSTOMER AUTONOMY"|"CUSTOMER VALUE"|"RETENTION"|"EXPANSION"|"BUSINESS VALUE";
export type AdoptionSemantic = {term:AdoptionSemanticTerm;definition?:string;sourceStatus:CompletenessStatus;source?:SourceReference};
export type AdoptionConcept = {
  id:string;
  title:string;
  purpose:string;
  trigger?:string;
  ...
  sourceLocators:SourceReference[];
  contentClass:ContentClass;
  completeness:Partial<Record<AdoptionLearningCompletenessDimension,CompletenessStatus>>;
};
export type TimeToValueModel = {
...
export type SourceRelationship = {element:string;status:"SUPPORTED"|"PARTIAL"|"MISSING";rationale:string;source:SourceReference};
export type OperatingExample = {name:string;classification:"EXAMPLE";details:Record<string,string|string[]>;source:SourceReference};
export type GrowthRelationship = {from:string;to:string;classification:"SOURCE FACT"|"ASSESSMENT INTERPRETATION"|"HYPOTHESIS"|"SOURCE MISSING";rationale:string;source:SourceReference;validationStatus:ValidationStatus};
export type NorthStarSupport = {candidate:string;contentClass:"hypothesis";supportStatus:"SUPPORTED"|"PARTIAL"|"MISSING";rationale:string;source:SourceReference;validationStatus:ValidationStatus};
export type FeedbackSignal = {signal:string;sourceRole:string;trigger?:string;format?:string;frequency?:string;relatedOutcome?:string;decisionEnabled?:string;artifact?:string;source:SourceReference};
...
export type LearningDecision = {decision:"SCALE"|"CONTINUE"|"ADJUST"|"EXPLORE"|"STOP";trigger:string;evidence:string[];participants:string[];decisionOwner?:string;output:string;revisitCondition?:string;source:SourceReference;validationStatus:ValidationStatus};
...
export type Chapter = {
  ...
  adoptionSemantics?:AdoptionSemantic[];
  adoptionConcepts?:AdoptionConcept[];
  timeToValue?:TimeToValueModel;
  customerOutcomeConnections?:SourceRelationship[];
  operatingExamples?:OperatingExample[];
  growthRelationships?:GrowthRelationship[];
  northStarSupport?:NorthStarSupport;
  feedbackSignals?:FeedbackSignal[];
  signalLearningChain?:SourceRelationship[];
  learningConcepts?:LearningConcept[];
  learningDecisions?:LearningDecision[];
  feedbackLoopConnections?:SourceRelationship[];
