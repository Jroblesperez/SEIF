...
export function SearchDialog({open,onClose}:{open:boolean;onClose:()=>void}){
  ...
  const results=useMemo(()=>{
    ...
    return {pages:chapters.filter(c=>(c.title+c.summary+c.takeaway+c.sections.map(section=>section.body??section.blocks?.map(block=>block.type==="paragraph"?block.text:block.type==="list"?block.items.join(" "):[block.table.caption,...block.table.headers,...block.table.rows.flat()].join(" ")).join(" ")??"").join(" ")).toLowerCase().includes(s)).slice(0,7),terms:glossary.filter(t=>(t.term+t.definition+t.example).toLowerCase().includes(s)).slice(0,7)};
  },[q]);
  if(!open)return null;
